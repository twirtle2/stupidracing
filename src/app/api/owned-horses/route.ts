...
/**
 * Given a list of asset IDs and an indexer URL, fetch details and filter for horses.
 */
async function findHorses(ownedAssetIds: number[], indexerUrl?: string) {
  const horseAssets = [];
  const chunkSize = 10;
  ...
  for (let i = 0; i < ownedAssetIds.length; i += chunkSize) {
    ...
    const results = await Promise.all(
      chunk.map((id) => fetchAsset(id, indexerUrl))
    );
    ...
    for (const asset of results) {
      ...
      if (isHorse) {
        const metadata = await fetchArc69Metadata(asset.index, indexerUrl);
        horseAssets.push({
          assetId: asset.index,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Always try to fetch from the configured network first
    const accountAssets = await fetchAccountAssets(address);
    const ownedAssetIds = accountAssets
      .filter((a) => a.amount && a.amount > 0)
      .map((a) => a["asset-id"]);

    let horseAssets = await findHorses(ownedAssetIds);

    // If on TestNet and no horses found, fallback to MainNet
    if (isTestNet && horseAssets.length === 0) {
      console.log(
        `[owned-horses] No horses on TestNet for ${address}, falling back to MainNet`
      );
      const mainnetAssets = await fetchAccountAssets(
        address,
        MAINNET_INDEXER_URL
      );
      const mainnetOwnedIds = mainnetAssets
        .filter((a) => a.amount && a.amount > 0)
        .map((a) => a["asset-id"]);

      horseAssets = (await findHorses(mainnetOwnedIds, MAINNET_INDEXER_URL)).map(
        (h) => ({ ...h, mirrored: true })
      );
    }
